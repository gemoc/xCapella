...
public class HeterogeneousEngine extends AbstractExecutionEngine implements IConcurrentExecutionEngine{//extends ConcurrentExecutionEngine{
	...
	public SolverClock getCoordinationSolverClockFromMSE(MSE inMSE, CCSLKernelSolver aSolver){
		if(inMSE instanceof FeedbackMSE){
			String cname = getQualifiedName((Clock)((FeedbackMSE) inMSE).getFeedbackModelSpecificEvent().getSolverEvent());
			return _coordinationSolver.getSolverWrapper().getSolver().findClockByPath(cname);
		}
		else{
