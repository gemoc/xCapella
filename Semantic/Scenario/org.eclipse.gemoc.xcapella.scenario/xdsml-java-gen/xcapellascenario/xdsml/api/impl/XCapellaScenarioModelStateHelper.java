...
public class XCapellaScenarioModelStateHelper implements IK3ModelStateHelper{
	...
	public K3ModelState getK3ModelState(EObject model) {
		...
		while (allContentIt.hasNext()) {
			...
			Class<?> clazz =null;
			clazz = K3DslHelper.getTarget(org.gemoc.scenario.k3dsa.SystemFunctionAspect.class);
			if (clazz.isInstance(elem)) {
				ElementState elemState = theFactory.createElementState();
				elemState.setModelElement(elem);
				res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("isStarted", XCapellaScenarioRTDAccessor.getisStarted(elem));
				elemState.getSavedRTDs().add(n2v0);
				AttributeNameToValue n2v1 = new AttributeNameToValue("isReady", XCapellaScenarioRTDAccessor.getisReady(elem));
				elemState.getSavedRTDs().add(n2v1);
				AttributeNameToValue n2v2 = new AttributeNameToValue("isSuspended", XCapellaScenarioRTDAccessor.getisSuspended(elem));
				elemState.getSavedRTDs().add(n2v2);
				AttributeNameToValue n2v3 = new AttributeNameToValue("isStopped", XCapellaScenarioRTDAccessor.getisStopped(elem));
				elemState.getSavedRTDs().add(n2v3);
				AttributeNameToValue n2v4 = new AttributeNameToValue("runCycles", XCapellaScenarioRTDAccessor.getrunCycles(elem));
				elemState.getSavedRTDs().add(n2v4);
			}
			clazz = K3DslHelper.getTarget(org.gemoc.scenario.k3dsa.AbstractEndAspect.class);
			if (clazz.isInstance(elem)) {
				ElementState elemState = theFactory.createElementState();
				elemState.setModelElement(elem);
				res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("occ", XCapellaScenarioRTDAccessor.getocc(elem));
				elemState.getSavedRTDs().add(n2v0);
			}
		}
		return res;
	...
	public void restoreModelState(K3ModelState state) {
		for(ElementState elemState : state.getOwnedElementstates()) {
			for(Object o : elemState.getSavedRTDs()) {
				...
				try {
					if (n2v.value != null) {
						Method m = XCapellaScenarioRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass());
					...
					for(Class<?> c : n2v.value.getClass().getInterfaces()) {
						...
						try {
							m = XCapellaScenarioRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass().getInterfaces()[0]);
							m.invoke(null, elemState.getModelElement(), n2v.value);
